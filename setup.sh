...
case "$OS" in
  ...
  Linux)
    ...
    ;;
  MINGW*|MSYS*|CYGWIN*)
    err "Native Windows is not supported. Use WSL (Windows Subsystem for Linux)."
    exit 1 ;;
  *)      err "Unsupported OS: $OS"; exit 1 ;;
esac

# Portable timeout (stock macOS doesn't have GNU timeout)
if ! command -v timeout &>/dev/null; then
  timeout() { local dur="$1"; shift; perl -e 'alarm shift; exec @ARGV' "$dur" "$@"; }
fi
...
if ! $SKIP_VOICE; then
  ...
  if command -v rec &>/dev/null || command -v sox &>/dev/null; then
    ok "sox ($(command -v rec 2>/dev/null || command -v sox))"
  else
...
if [ -z "${WHISSLE_API_TOKEN:-}" ] && [ -z "${WHISSLE_USER_ID:-}" ]; then
  ...
  if [ -z "$CRED_INPUT" ]; then
    err "No credential provided. Get a token at lulu.whissle.ai/access"
    exit 1
  fi
...
mkdir -p "$TOKEN_DIR"
{
  echo "# Lulu credentials — generated by setup.sh on $(date +%Y-%m-%d)"
  printf 'WHISSLE_API_TOKEN=%s\n' "${WHISSLE_API_TOKEN:-}"
  printf 'WHISSLE_USER_ID=%s\n' "${WHISSLE_USER_ID:-}"
  printf 'WHISSLE_USER_NAME=%s\n' "${WHISSLE_USER_NAME:-}"
  printf 'WHISSLE_LOCATION=%s\n' "${WHISSLE_LOCATION:-}"
} > "$TOKEN_FILE"
...
ENDJSON
  ...
  if $DO_CLAUDE_CODE; then
    ...
    [ -n "${WHISSLE_USER_NAME:-}" ] && HOOK_ENV="$HOOK_ENV WHISSLE_USER_NAME='$(echo "$WHISSLE_USER_NAME" | sed "s/'/'\\\\''/g")'"
    [ -n "${WHISSLE_LOCATION:-}" ] && HOOK_ENV="$HOOK_ENV WHISSLE_LOCATION='$(echo "$WHISSLE_LOCATION" | sed "s/'/'\\\\''/g")'"

    PROMPT_HOOK_CMD="$HOOK_ENV $PYTHON $HOOKS_DIR/prompt-submit.py"
    SESSION_HOOK_CMD="$HOOK_ENV $PYTHON $HOOKS_DIR/session-start.py"

    HOOK_PROMPT_CMD="$PROMPT_HOOK_CMD" \
    HOOK_SESSION_CMD="$SESSION_HOOK_CMD" \
    SETTINGS_PATH="$CLAUDE_SETTINGS" \
    python3 -c "
import json, os

settings_path = os.environ['SETTINGS_PATH']
prompt_cmd = os.environ['HOOK_PROMPT_CMD']
session_cmd = os.environ['HOOK_SESSION_CMD']
...
hooks['UserPromptSubmit'] = [{'hooks': [{'type': 'command', 'command': prompt_cmd, 'statusMessage': 'Lulu: reading emotion + intent...'}]}]
hooks['SessionStart'] = [{'hooks': [{'type': 'command', 'command': session_cmd, 'statusMessage': 'Lulu: loading your personality...'}]}]
